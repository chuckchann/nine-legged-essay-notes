...
- GC开始将所有对象都置为黑色（之后不再进行二次扫描，无需STW）
- GC期间任何创建的新对象都置为黑色
- 被删除的对象都置为灰色
- 被添加的对象置为灰色

Golangv1.8版本的混合写屏障的机制满足弱三色不变式，结合了删除写屏障和插入写屏障的的优点，只需要在开始时并发扫描各个goroutine的栈，使其变黑并一直保持，这个过程不需要STW。而标记结束后，因为栈在扫描后始终是黑色的，也无需再进行re-scan操作了，减少了STW的时间。
